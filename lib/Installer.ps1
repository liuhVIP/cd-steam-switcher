...
function Install-DownloadedVersion { param([string]$Source,[string]$GameDir,[switch]$DryRun)
 ...
 Write-Progress -Activity '安装历史版本' -Completed;Write-Host '安装完成，下载缓存默认保留。' -ForegroundColor Green;$remove=(Read-Host '是否删除下载缓存？(Y/N，默认 N)').Trim().ToUpperInvariant();if($remove -eq 'Y'){Remove-Item -LiteralPath $Source -Recurse -Force;Write-Host '已删除下载缓存。' -ForegroundColor Green}else{Write-Host ('已保留下载缓存: '+$Source) -ForegroundColor Cyan};$plan
}
