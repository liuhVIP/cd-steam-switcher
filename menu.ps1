        ...
        if($activeTask){
            ...
            if($finished){
                ...
                if(Test-SteamDepotDownloaded $autoContent){
                    $msg=if($finished.Manifest){'检测到已完成下载: Build '+$finished.BuildId+' / manifest '+$finished.Manifest}else{'检测到 Depot 内容已完整写入（Steam 尚未记录 finished update）'};Write-StepOk $msg
